...
export const addAllStudents = (fileFormData) => async (dispatch) => {
  try {
    const { data } = await api.addAllStudents(fileFormData)

    // Dispatch the action to indicate success
    dispatch({ type: ADD_ALL_STUDENTS, payload: true })
    ...
    alert(
      'Error adding all students. Please check the file format and try again.'
    )
  }
}
